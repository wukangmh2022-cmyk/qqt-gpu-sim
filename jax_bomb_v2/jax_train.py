...
def reward_from_events(dmg, alive_before, alive_after, hp_after, done,
                       crate_add, newly, walls_destroyed, crate_coef,
    ...
    # crate_add = 实际成长档位（普通箱 +1、超级箱 +4、问号箱 0-2 随机；
    # 满级截断后按真实成长计），奖励 = coef × 档位
    rew = rew + crate_coef * crate_add.astype(jnp.float32)
...
def hl_gauss_value_loss(v_logits, targets, v_min=V_MIN, v_max=V_MAX,
    ...
    """HL-Gauss categorical cross-entropy（量程按 round-1 老奖励 ±20，σ≈5 桶宽）。"""
    ...
    **本局首次到达**（共享 visited 掩码，done 清零）→ +explore_coef。这是
    ...
    def one_step(carry, _):
        ...
        rew = reward_from_events(
            ...
            info["crate_add"], newly, info["walls"], crate_coef, explore_coef,
...
def ppo_update_lsgd(params, opt, opt_state, arch, batch, key, minibatch,
    ...
    for _ in range(epochs):
        key, ek = jrandom.split(key)
        # 与 ppo_update 同款组合：前半 Actor(Top-|A|) + 后半 Critic(均匀)，
        # 修复 ae88ee3 版 LSGD 组合被忽略的死代码
        n_mb_c = n_keep // mb_half
        perm_a = jrandom.permutation(ek, n_keep)
        idx_a = actor_idx[perm_a].reshape(n_mb_c, mb_half)
        critic_perm = jrandom.permutation(ek, total)[:n_keep]
        idx_c = critic_perm.reshape(n_mb_c, mb_half)
        idx = jnp.concatenate([idx_a, idx_c], axis=1)
        n_mb = idx.shape[0]
