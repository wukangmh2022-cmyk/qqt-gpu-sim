...
记录 (obs7, teacher_logits)。

设计（与 jax 训练侧对齐，全批量向量化跑 DCU）：
  - 环境 = open 空场 1v1，SimConfig 数值对齐 jax（speed=7.56/blast=7/
    max_steps=1800/invuln_ticks=30/max_hp=5/max_bombs=10，map_mode=open）
  - num_envs=N 并行：每 tick 推进 N 局，teacher + 对手都是批量前向
  - 每 tick 存 N 组、双视角各一：
      obs7 : jax make_obs 的 7 通道布局（**不含危险图**）
             ch0 我位置 splat  ch1 我泡 fuse/FUSE  ch2 对手位置
             ch3 对手泡 fuse  ch4 墙(0)  ch5 泡威力 blast/BLAST  ch6 t/1800
      logits : teacher 的 (move_logits[5], bomb_logits[2])，mask 后
  ...
  - 存储：npz，每对手一个文件：obs7 (T,2,7,13,13) uint8×255、
    logits (T,2,7) float16（T = 总 tick×N 帧，双视角排成 2 块）。
...
#   ch3 对手泡=共享3  ch4 墙=共享4  ch5(替换) ch6 进度=共享6
VIEW_MAP = [0, 2, 1, 3, 4, 6]


def make_cfg() -> SimConfig:
    """对齐 jax 环境数值（化繁为简正式版：成长满级 open 空场）。"""
...
def teacher_logits(net: ActorCritic, obs14, mm, bm, pid: int):
    """teacher 批量前向，返回 mask 后 logits (N,5)/(N,2)。恒 pid=0 视角。"""
    with torch.no_grad():
        dm, db, _ = net.dists(obs14, mm[:, pid].bool(), bm[:, pid].bool(), 0)
...
def obs7_batch(sim: BatchedSim, obs14: torch.Tensor) -> torch.Tensor:
    ...
    # 视角重排 + 拼接
    #   pid0 视角：我=共享0/2、对手=共享1/3 → VIEW_MAP [0,2,1,3,4,6]
    #   pid1 视角：我=共享1/3、对手=共享0/2 → [1,3,0,2,4,6]
    view1 = [1, 3, 0, 2, 4, 6]
    o7 = torch.stack([
        obs14[:, VIEW_MAP].clone(),
        obs14[:, view1].clone(),
    ], dim=1)                                                 # (N,2,7,H,W)
    # ch5 换成泡威力图（两视角共享同一张 blast 图）
    o7[:, :, 5] = ch5[:, None].expand(n, 2, H, W)
    return o7
...
def run_batch(args, teacher: ActorCritic, opp_net, opp_bot, device,
    ...
    obs_arr = np.concatenate(obs_acc).astype(np.uint8)          # (T,2,7,H,W)
    lg_arr = np.concatenate(lg_acc).astype(np.float32)          # (T,7) 存 fp32，防溢出
    T = obs_arr.shape[0]
    ...
    print(f"[{name}] {T:,} 帧 -> {path} "
          f"({T*2*7*H*W*1.0/1e6:.0f} MB) {dt:.1f}s", flush=True)
    return T
