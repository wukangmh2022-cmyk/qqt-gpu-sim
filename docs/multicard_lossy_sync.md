...
- 模型 7.46M 参数 ≈ 29.8MB fp32。一次全量 allreduce 在 N 卡 ring 下每卡移动 `2×S×(N−1)/N` 字节
  （2 卡 = 29.8MB，20 卡 = 56.6MB）。
- 每迭代 minibatch 数 = `(2×envs_per×num_steps/minibatch) × epochs`，生产配置 = **1024 个**。
- 每个 minibatch 一次全量梯度同步 → 20 卡时 **~58GB/迭代/卡** → 通信 100s+ 级，完全不可行。
...
- 通信量：同步次数从 1024 降到 1024/K。K=256 → 4 次/迭代 ≈ 226MB（20 卡）/ 119MB（2 卡）。
