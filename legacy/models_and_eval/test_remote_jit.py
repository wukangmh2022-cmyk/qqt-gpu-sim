...
step_jit = jax.jit(jax.vmap(lambda s, a, kk: step(s, a, kk, return_info=True)))

def test_map(map_name, weights, num_envs=128, num_steps=600):
    ...
    for t in range(num_steps):
        ...
        new_states, done, info = step_jit(states, env_acts, keys)
    ...
    print(f"=== {map_name} (打静止靶子, 完局数={tot}) ===", flush=True)
...
print("1. 纯空场景道场 (Stage 0 地图):", flush=True)
test_map("纯空场景道场", weights="empty=1.0")

print("\n2. 包含大量宝箱的复杂地图 (用户截图中的场景):", flush=True)
