# 🌿 `jev` 分支说明：TypeSafe Jev 大模型自主决策架构

> **本分支定位**：探索基于 **TypeSafe Jev（System One）大语言模型** 的纯认知驱动泡泡堂对战 AI。
> 坚决摒弃传统底层死板硬编码规则（如底层私自放泡、人工写死宏动作等），将游戏的**全量客观规则、空间物理与动态时序 100% 完整透明地喂给大模型**，由大模型自主推演微观走位、发育升级与攻防博弈。

---

### 🧠 Jev 大模型架构核心特性（已收敛至旗舰版 `web/jev_grid_ai.js`）

1. **15×13 全图高维空间矩阵输入（Full 2D Grid Observation）**：
   - 将整张地图字符化编码为大语言模型原生理解的二维矩阵，辅以连通分量（Connected Components）分析，隔离不可达盲区（`?`）；
   - **道具箱精细化区分**：矩阵中明确区分道具种类——`s`（移速药水，落后时最高发育优先级）、`f`（火焰威力）、`b`（炸弹上限）、`c`（随机问号箱），彻底告别原版模糊笼统的单一字母 `C`。

2. **0~9 危险倒计时热力图（Danger Countdown Heatmap）**：
   - 将全图所有可能起火的十字火线量化为精确到毫秒的 `0~9` 倒计时（`0`=当前烈焰燃烧中、`1~3`=900ms 内临界引爆死线、`4~6`=中期、`7~9`=延迟安全、`.`=绝对安全通行）；
   - **连锁引爆传播算法（Chain Detonation）**：动态计算先起火的老泡引爆后续炸弹的因果时间线，拉平整条引爆链。

3. **胜负与运行时潜在机制全量透传（Match & Latent Rules）**：
   - **血量与一击必杀警示**：注入触火扣 1HP、0HP 淘汰判负、1HP 处于碰火即死状态（SURVIVAL ALERT）；
   - **烈焰余威（`blastLinger`）**：明确告知模型倒计时归 0 后仍有 0.3s（250~300ms）的致命火线残留，切勿立刻踩入；
   - **炸弹库存机制**：完整上报最大容量 `bombs_cap`、在场活跃 `active_bombs`、手里立即可用 `available_bombs`，告知爆炸熄灭后槽位归还机制；
   - **泡泡连锁引爆因果与战术**：传授连珠炮进攻技巧（快速贯穿走廊）与防老泡近身自爆警示。

4. **高维坐标直出与纯净决策空间（Zero Hardcoded Rules）**：
   - **输出维度**：大模型直接直出目标行坐标 `target_row`（r0~r12）、列坐标 `target_col`（c0~c14）、战略意图 `strategic_intent`（追猎对手 / 发育吃道具 / 炸砖开路 / 避险撤离）与下子动作 `bomb_decision`（`plant_bomb_now` / `hold_bomb`）；
   - **拒绝底层越权**：底层严禁自行判断贴砖自动下蛋，决策权 100% 交由大模型。
...
2. **浏览器体验**：
   ...
   - 敌人 AI 下拉菜单选择：`🧠 TypeSafe Jev（高维坐标全图版）`；
   - 支持勾选「观战（AI vs AI）」欣赏 Jev 与时空 A* / Hunter / ViT 的实时死斗。
3. **自动化基准评测**：
   ```bash
   node scripts/eval_jev.js
   ```
...
- **高水平自博弈必现“对峙纳什陷阱”**（阶段四）：双方避险与闪避能力越强，主动靠近放炮的边际风险越高，纯自博弈极易塌缩至安全距离静止对峙（放炮骤降至 2.3 颗/局，挂机率 48%）→ **必须依赖多态对手联赛池（逃跑怪 + 漫游怪 + 历史自对弈池 + 死靶）打破镜像对称，配合空间压迫与对峙破局机制彻底根治**。
