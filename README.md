...
- **提前终止与止损复盘**：实际运行至 31.9B 步（保留 `1.1B/7.5B/22.6B/31.9B` 四档快照，占原定规划约 12%）。训练推进至 30B 深水区后，发现标量 Critic 出现严重的价值估计饱和与长程归因模糊，导致策略探索停滞、陷入“0 炮自保低谷”与发呆纳什陷阱；继续盲跑剩余 230B 步已无增益。团队果断叫停阻断算力浪费，直接驱动了第三轮 P0 体系重构（HL-Gauss 128 桶价值头 + Top-25% 优势过滤 + Patch 3）。
