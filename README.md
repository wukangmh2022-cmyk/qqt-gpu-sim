...
### 拟人化控制与手感对齐（本体感知与外感知解耦）

- **纯动作降频（5Hz 动作重复）的物理失效根因**：
  在 QQ 堂连续物理引擎中，角色每 tick 位移约 0.56~0.65 格。若生硬采用 `action_repeat = 2`（5Hz 动作锁死），单次按键最小位移强制被拉大至 1.12~1.30 格。由于道具拾取判定半径与走廊缝隙仅约 0.5 格，**最小位移大于目标容差**必然引发控制理论中的**极限环超调震荡（Limit Cycle Oscillation）**：在道具附近反复横跳冲过头、无法急刹、且在 2-tick 盲区中频发走入火区自杀（承伤高达 26 点，胜率跌至 60%）。
- **人类手感的本质：本体感觉 10Hz 微步 + 视神经延迟 200ms**：
  人类玩家的 200ms 反应是指**视神经与视觉中枢的处理延迟（Exteroception）**，而手指点按键盘、感知自身角色走位是**高频本体感觉（Proprioception）**。人类可以通过快速点按方向键走小碎步和即刻急刹，绝不会按下一个键强制黏住 200ms。
- **双环解耦控制架构（借鉴 ETH ANYmal 与 AlphaStar）**：
  在推理层（[web/sim.js](web/sim.js)）实现感知解耦：
  1. **本体感觉环（10Hz 实时）**：`ch0`（自身坐标）与自身属性状态（血量、存活、属性）保持零延迟当前 tick 实时感知，保障 0.56 格微步调整与即刻急刹；
  2. **外感知视觉环（100~200ms 滞后）**：`ch1~ch14`（外部危险图、对手位置、炸弹、箱子）通过滑动 FIFO 缓冲区滞后 1~2 tick（150ms 滞后 100ms，200ms 滞后 100~200ms）；
  3. **效果收益（`params_it00000683_ema` 自杀降低版实测）**：
     - **胜率**：从 60% 跃升至 **100.0%**（对战高级规则猎人 5 战 5 胜）；
     - **吃箱数**：从 52 个暴增至 **145 个**（+178%），彻底消除过冲震荡；
     - **承伤与自杀**：承伤骤降 70%，消除盲目踩火自杀，残局多呈现**满血击杀**；
     - **全系通用**：全库 137 个模型（119 个 Transformer 宗师版 + 18 个 MLP/CNN）在 Web 端切换 150ms/200ms/250ms 时全部自动生效。
