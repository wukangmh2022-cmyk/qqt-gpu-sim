...
本轮技术重构直接沉淀出两大关键底模，无缝衔接阶段四：
1. **基础开荒底模 `it349`**（累计 2.93B 步）：掌握全图开荒炸墙、拾取道具与基础避险身法；
2. **激进破拆底模 `aggr419`**（在 `it349` 基础上续训 3.94B 步，累计 6.87B 步）：强化主动压迫、破拆走位与贴身肉搏，成为阶段四联赛破局的基线底模。
