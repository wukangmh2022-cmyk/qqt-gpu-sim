...
| **逃跑拉扯bot** | 快速拉开距离、动态避弹风筝逃跑 | 71 胜 20 平 9 负（**91.4%**） | 65 胜 25 平 10 负（**90.0%**） |
| **规则 BFS 猎人** | 全局多源 Dijkstra 追击，高频落泡压制 | 63 胜 0 平 1 负（**98.4%**） | 62 胜 2 平 0 负（**100.0%**） |

> 注：不败率统计为「胜局 + 超时平局」。规则 BFS 猎人数据基于 64 局无头全量实测（全场景唯一负局源自 21 号「沙漠之洲」厚重砖墙超时血差判负；开阔地 2 平为双亡同归）。人类对战 50 局实测中 AI 战绩为 43 胜 5 平 2 负（胜率 86.0%，不败率 96.0%），失利源于死角自陷与一血险负。
