...
> **💡 奖励函数三阶段演进脉络**：
> `it349（零和生存与开荒）`：纯生命差 `r = (造成−受到)/5` + 基础箱子拾取，建立全图开荒与规避自伤的身法基底 ➔ `aggr419（激进破拆与惩罚消极）`：高额破墙奖励（brick 0.30）+ 超时双输（-2.0）+ 互损暴毙重罚，逼出开阔地贴身肉搏战意 ➔ `it831（拆除枷锁与纯胜负决胜）`：清零互损与暴毙惩罚（彻底解除畏手畏脚束缚）+ 顶格胜负奖（win=10.0）+ 动态静止惩罚，配合多态联赛对手池终结纳什对峙死锁。
