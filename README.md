...
> `it349（基础开荒底模）`：**胜负大奖 ±10.0** + 单步零和生命差 `(造成−受到)` 引导基础对攻 + 基础吃箱，建立全图开荒与规避自伤的身法基底 ➔ `aggr419（激进破拆底模）`：**胜 6.0 负 5.0（换血险胜 3.5）** + 高额破墙奖励（brick 0.30）+ 超时双输（-2.0）与暴毙重罚，逼出开阔地贴身肉搏战意 ➔ `it831（宗师破局长训）`：**恢复顶格胜负 ±10.0** + 彻底清零互损与暴毙惩罚（解除畏手畏脚束缚）+ 动态静止惩罚，配合多态联赛对手池终结纳什对峙死锁。
