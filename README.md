# qqt-gpu-sim — QQ 堂风格 1v1 格斗：GPU 批量模拟器 + 自博弈 PPO
...
|---|---|:---:|:---:|
| **人类高玩（实测）** | QQ 堂十年玩家（非顶尖选手），可代表 90% 的人类玩家水平 | 40 胜 7 平 3 负（**94.0%**） | 43 胜 5 平 2 负（**96.0%**） |
| **高级时空 A*** | 多步引信推演、时空 A* 避险与包抄 | 32 胜 0 平 0 负（**100.0%**） | 32 胜 0 平 0 负（**100.0%**） |
| **逃跑拉扯bot** | 快速拉开距离、动态避弹风筝逃跑 | 28 胜 2 平 2 负（**93.8%**） | 29 胜 1 平 2 负（**93.8%**） |
| **规则 BFS 猎人** | 全局多源 Dijkstra 避险寻路，严格安全约束下低频谨慎落泡 | 63 胜 0 平 1 负（**98.4%**） | 62 胜 2 平 0 负（**100.0%**） |

> 注：不败率统计为「胜局 + 平局」（平局多为 1 血残局双方同归于尽，极少超时）。人类对战 50 局实测中 AI 战绩为 43 胜 5 平 2 负（胜率 86.0%，不败率 96.0%），失利源于死角自陷与一血险负。权重文件开源归档于 `ckpt/`（JAX 原生 pkl）与 `web/models/`（Web/ONNX）。
...
#### 第二次大训练（2026-08-21 ~ 08-27）：原定 260B（实跑至 31.9B 提前止损）
...
- 原定目标 8 卡 260B（16384 envs，8.39M 步/iter，ITERS=31000）；中途 08-26 引入 10 机 × 2 卡从 it889 热启动续训；
- **提前终止与止损复盘**：实际运行至 31.9B 步（保留 `1.1B/7.5B/22.6B/31.9B` 四档快照，占原定规划约 12%）。训练推进至 30B 深水区后，发现标量 Critic 出现严重的价值估计饱和与长程归因模糊，导致策略探索停滞、陷入“0 炮自保低谷”与发呆纳什陷阱；继续盲跑剩余 230B 步已无增益。团队果断叫停阻断算力浪费，直接驱动了后续 P0 体系重构。

#### P0 架构重构与前序底模确立（it349 → aggr419，6.87B 步）

针对第二次大训练暴露的标量 Critic 饱和与局域微操迟钝，全面废除旧参数体系，落地 P0 级技术改造：
- **零和生命演进奖励**：废除人工碎步奖励，改用纯对局生命差比值 `r = (造成伤害 − 受到伤害) / 5`；
- **HL-Gauss 分布式价值头**：标量 Critic 升级为 128 桶分类头（区间 `[-20, 20]`，σ=1.5，交叉熵优化），彻底解决价值估计饱和与长程归因模糊；
- **Actor Top-25% 优势过滤**：仅从高质量博弈样本中汲取策略梯度；
- **Patch 4 → 3**：单图切分为 25 个 Patch Token，大幅增强局域微操与连泡感知。

本轮技术重构直接沉淀出两大关键底模，奖励机制逐步演进：
1. **基础开荒底模 `it349`**（累计 2.93B 步）：以纯零和生命差与基础箱子探索为驱动，建立全图开荒、拾取道具与规避自伤的身法基底；
2. **激进破拆底模 `aggr419`**（在 `it349` 基础上续训 3.94B 步，累计 6.87B 步）：转向高额破墙奖励（brick 0.30）并叠加超时双输（-2.0）与互损暴毙重罚，强行逼出开阔地极速贴身肉搏战意。
...
2. **主动进攻与对峙破局机制**：
   - **拆除避战枷锁与顶格胜负激励**：彻底清零同归于尽与互损惩罚（mutual_hit=0.0, double_death=0.0），解除放泡自保束缚；恢复顶格胜负奖励（win_bonus=10.0, trade_win=5.0）；
   - 引入对峙距离衰减势能与动态静止惩罚（idle_penalty=0.015，长时间不靠近且不放泡施加负反馈）；
...
> **💡 奖励函数三阶段演进脉络（终局胜负始终是 ±10 分核心信号，演进本质在于中间步态塑形与消极惩罚的调控）**：
> `it349（基础开荒底模）`：**胜负大奖 ±10.0** + 单步零和生命差 `(造成−受到)` 引导基础对攻 + 基础吃箱，建立全图开荒与规避自伤的身法基底 ➔ `aggr419（激进破拆底模）`：**胜 6.0 负 5.0（换血险胜 3.5）** + 高额破墙奖励（brick 0.30）+ 超时双输（-2.0）与暴毙重罚，逼出开阔地贴身肉搏战意 ➔ `it831（宗师破局长训）`：**顶格胜奖 +10.0 / 负 -5.0（换血险胜 +5.0）** + 彻底清零互损与暴毙惩罚（解除畏手畏脚束缚）+ 动态静止惩罚，配合多态联赛对手池终结纳什对峙死锁。
