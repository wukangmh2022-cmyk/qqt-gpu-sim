...
强化学习智能体在复杂对战场景中的代表性高阶博弈表现与底层游戏物理机制演示：
...
| **半身连泡**<br>Half-Body Chain | <img src="docs/gameplay/kill_half_body_chain.gif" width="360" alt="半身连泡"> | **并排连泡压制**：布设并排双泡诱发平行水柱覆盖缝隙，封锁半身安全区压制对手并尝试击杀（源自空场景实战前 5 秒）。 |
...
| 训练量口径 | **终生累计约 17.30B 步（173 亿步）**：底模 it349（2.93B）→ 破拆 aggr419（3.94B）→ 宗师 it831（10.43B）。<br>本次阶段四单轮跑量：**831 iter × 12.57M 步/iter = 10,433,617,920 步（约 10.43B 步）**。 |
...
- **纯自博弈的开阔地对峙纳什陷阱**：双方避险微操越高、5 血容错越大，主动在开阔地放炮的暴露风险越高，纯自博弈必塌缩至安全距离外相互静止对峙（放炮骤降至 2.3 颗/局，发呆率超 48%）。**坚决拒绝非原版的毒圈缩圈，全面通过多态对手联赛池（静态死靶 + 漫游走位 + 智能拉扯）打破镜像对称，强行逼出跨图搜寻与下泡封路的追猎本能**。
- **初始血量域随机化击碎长盘苟活庇护所**：固定 5 血使智能体倾向于防守反击与消极避战。**引入初始血量域随机化（40% 1-HP 生死局 + 30% 2~3 HP 快节奏死斗 + 15% 逆境非对称血量）**，1 血生死局使单次命中价值与存活惩罚相对权重急剧放大，原地发呆等同自杀，逼出开阔平原多向身位压迫与连环下泡的闪电终结能力。
- **规则对手陪练的纯净梯度屏蔽（mask_bot_advantages）**：混入规则 Bot（如 FleeBot/IdleBot）能有效提供进攻信号，但若反向传播 Bot 席位的轨迹，会导致策略网络学到规则 Bot 的被动或负向行为（策略污染）。**在反向传播时将 Bot 席位优势度强制置零、目标回报对齐当前估值，Top-k 优势过滤中将 Bot 权重视为负无穷（-1e9）**，确保模型 100% 仅从“AI 追猎击杀 Bot”的高价值样本中汲取梯度。
- **规则 Bot 的向量化防自灭机制**：传统手写 Hunter 在死角窄道容易就地盲目落雷自爆，导致自博弈 AI 躺赢产生虚假胜率。**在 JAX 环境中注入纯张量化防自灭判定（仅当当前位置存在 ≥2 个合法可逃生方向时才允许落雷）**，死角狭道只走位不放雷，彻底剥夺 AI 躺赢空间。
- **Local SGD 大规模集群有损同步（K=32 周期参数平均）**：24 卡跨机若做强一致 AllReduce 极易被网络抖动拖垮。**采用 Local SGD 周期参数同步（K=32 步跨机 pmean 一次 fp32 参数）**，通信开销近乎为零，集群算力利用率飙升至 46.6 万 SPS 历史巅峰，且各副本局部的微小参数分歧天然充当了自博弈的多样性探索噪声。
- **掉血爆属性守恒与发育保底底噪（退火防遗忘）**：后期奖励退火若把吃道具/开荒奖励彻底归零，百亿步深水区训练中 AI 会逐渐遗忘吃速度鞋与药水发育的本能；若乱加奖励又会导致开箱刷分。**设置保底底噪（crate_floor=0.10）永久保留发育直觉，配合掉血爆属性守恒（被炸扣 2 层补 2 箱，守恒散落）**，既杜绝刷分，又保证逆境翻盘本能长盛不衰。
- **AI 拟真反应调速与抗震荡架构（推理即行动 0 滞后）**：为配合人类手速若在前端引入动作 FIFO 时延队列，时延会引入控制超调与相位滞后，导致 AI 在网格边缘剧烈左右晃荡（极限环震荡）。**必须采用推理即行动（0 时延下达）+ 原生 inferEvery 降频架构**：推理决策帧即时下达动作，非推理冷却期顺滑维持移动惯性且放泡脉冲清零，既为人类玩家留出博弈窗口，又彻底杜绝左右摇摆震荡。
