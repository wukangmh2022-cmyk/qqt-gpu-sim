#!/bin/bash
# 监控 10 台训练：每 60s 拉各节点 train_r<i>.log 状态；进程在但日志 5 分钟
# 无更新 → 标 [疑似卡死]（RCCL 同步屏障下某台掉线=全体卡死，需降级重启）。
# 用法: bash watch_10nodes.sh nodes.txt
# 注意: 重新 launch 前先停掉本脚本（launch 会 rm -rf /tmp/ndrun）。
set -u
NODES_FILE="${1:?用法: watch_10nodes.sh nodes.txt}"
WORK=/tmp/ndrun
[ -d "$WORK" ] || { echo "先跑 launch_10nodes.sh 生成封装"; exit 1; }
...
while true; do
  ...
  echo "=== 掉线降级：全部 pkill → 用存活节点重写 nodes.txt → launch_10nodes.sh（自动接续断点）==="
  sleep 60
done
