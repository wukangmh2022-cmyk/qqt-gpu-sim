#!/usr/bin/env bash
# ==============================================================================
# 第五阶段：12 节点 24 卡 DCU 12 小时 5Hz 宗师全图长训
...
CONFIG_REL="configs/long_12h_stage5_5hz.toml"
...
rm -rf /tmp/jaxbomb_stage5 && mkdir -p /tmp/jaxbomb_stage5/scripts /tmp/jaxbomb_stage5/configs /tmp/jaxbomb_stage5/ckpt /tmp/jaxbomb_stage5/tests /tmp/jaxbomb_stage5/wheels
cp -r "$ROOT/jax_bomb" /tmp/jaxbomb_stage5/
cp -r "$ROOT/tests" /tmp/jaxbomb_stage5/
cp "$ROOT/scripts/cluster_selfcheck.py" /tmp/jaxbomb_stage5/cluster_selfcheck.py 2>/dev/null || true
cp "$ROOT/levels.json" /tmp/jaxbomb_stage5/levels.json
cp -r "$ROOT/configs"/* /tmp/jaxbomb_stage5/configs/
cp "$ROOT/ckpt/params_it00000831_ema.pkl" /tmp/jaxbomb_stage5/ckpt/ 2>/dev/null || cp "$ROOT/ckpt_local/params_it00000831_ema.pkl" /tmp/jaxbomb_stage5/ckpt/
cp /tmp/dcu_wheels/*.whl /tmp/jaxbomb_stage5/wheels/ 2>/dev/null || true

cat > /tmp/jaxbomb_stage5/start_rank.sh <<'EOSTR'
#!/bin/bash
RANK=$1
WORLD_SIZE=$2
MASTER=$3
...
EOSTR
chmod +x /tmp/jaxbomb_stage5/start_rank.sh

find /tmp/jaxbomb_stage5 -name "__pycache__" -type d -exec rm -rf {} + 2>/dev/null
(cd /tmp/jaxbomb_stage5 && tar czf /tmp/jaxbomb_stage5.tgz jax_bomb tests levels.json configs ckpt start_rank.sh wheels)
LOCAL_MD5=$(md5 -q /tmp/jaxbomb_stage5.tgz 2>/dev/null || md5sum /tmp/jaxbomb_stage5.tgz | awk '{print $1}')
echo "  ✓ 本地归档构建完成 MD5: $LOCAL_MD5"

echo "=== [3/4] 上传代码包至 Rank 0 并解压至各 Rank 工作区 ==="
"$WORK/scp_0" /tmp/jaxbomb_stage5.tgz /root/private_data/jaxbomb_stage5.tgz
echo "  ✓ 代码包已推送至 Rank 0"
