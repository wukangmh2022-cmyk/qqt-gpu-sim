#!/usr/bin/env bash
# ==============================================================================
# 15 节点 30 卡 DCU 8 小时超长训启动脚本 (Warm-Start 继承 it400 黄金战力)
# 由 configs/long_8h_15nodes_patch3_k32_warmstart.toml 全权驱动
# ==============================================================================
set -euo pipefail
...
CONFIG_REL="configs/long_8h_15nodes_patch3_k32_warmstart.toml"
...
echo "=== 已提取全部 $NW 台节点用于 30 卡 8 小时超长训 ==="

WORK="/tmp/long_8h_15nodes_work"
...
echo "=== [2/4] 打包并同步最新代码、配置与 it400 底模到全部 15 节点 ==="
rm -rf /tmp/jaxbomb_8h && mkdir -p /tmp/jaxbomb_8h/scripts /tmp/jaxbomb_8h/web/assets/maps /tmp/jaxbomb_8h/configs /tmp/jaxbomb_8h/ckpt
cp -r "$ROOT/jax_bomb" /tmp/jaxbomb_8h/
cp "$ROOT/scripts/cluster_selfcheck.py" /tmp/jaxbomb_8h/cluster_selfcheck.py
cp "$ROOT/levels.json" /tmp/jaxbomb_8h/levels.json
cp -r "$ROOT/configs"/* /tmp/jaxbomb_8h/configs/
cp "$ROOT/ckpt/params_it00000400.pkl" /tmp/jaxbomb_8h/ckpt/
find /tmp/jaxbomb_8h -name "__pycache__" -type d -exec rm -rf {} + 2>/dev/null
(cd /tmp/jaxbomb_8h && tar czf /tmp/jaxbomb_8h.tgz jax_bomb levels.json configs ckpt cluster_selfcheck.py)

for i in $(seq 0 $((NW-1))); do
  (
    "$WORK/scp_$i" /tmp/jaxbomb_8h.tgz /root/private_data/ >/dev/null 2>&1
    "$WORK/cmd_$i" "cd /root/private_data && rm -rf qqt-gpu-sim_r$i && mkdir -p qqt-gpu-sim_r$i && tar xzf jaxbomb_8h.tgz -C qqt-gpu-sim_r$i" >/dev/null 2>&1
    echo "  ✓ [rank $i] 代码与 it400 预训练底模部署完成"
  ) &
done
wait
...
echo "=== [4/4] 启动 15 节点 30 卡 8 小时超长训 (MASTER: ${MASTER}:29500, NW=$NW) ==="
# Rank 0
...
echo "=== 全部 $NW 个 Rank (30 张 DCU 卡) 已全量点火，8 小时超长训正式启动！ ==="
