#!/usr/bin/env bash
# ==============================================================================
# 9 节点 18 卡 DCU 激进进攻超长训启动脚本 (Warm-Start 继承 it102 黄金战力)
# 由 configs/long_8h_aggressive_warmstart.toml 全权驱动
# ==============================================================================
set -euo pipefail
...
CONFIG_REL="configs/long_8h_aggressive_warmstart.toml"
...
echo "=== 已提取全部 $NW 台节点用于 18 卡激进进攻长训 ==="

WORK="/tmp/aggressive_8h_work"
...
echo "=== [2/4] 打包并同步最新代码、配置与 it102 底模到全部 9 节点 ==="
rm -rf /tmp/jaxbomb_aggr && mkdir -p /tmp/jaxbomb_aggr/scripts /tmp/jaxbomb_aggr/web/assets/maps /tmp/jaxbomb_aggr/configs /tmp/jaxbomb_aggr/ckpt
cp -r "$ROOT/jax_bomb" /tmp/jaxbomb_aggr/
cp "$ROOT/scripts/cluster_selfcheck.py" /tmp/jaxbomb_aggr/cluster_selfcheck.py
cp "$ROOT/levels.json" /tmp/jaxbomb_aggr/levels.json
cp -r "$ROOT/configs"/* /tmp/jaxbomb_aggr/configs/
cp "$ROOT/ckpt/params_it00000102.pkl" /tmp/jaxbomb_aggr/ckpt/
find /tmp/jaxbomb_aggr -name "__pycache__" -type d -exec rm -rf {} + 2>/dev/null
(cd /tmp/jaxbomb_aggr && tar czf /tmp/jaxbomb_aggr.tgz jax_bomb levels.json configs ckpt cluster_selfcheck.py)

for i in $(seq 0 $((NW-1))); do
  (
    "$WORK/scp_$i" /tmp/jaxbomb_aggr.tgz /root/private_data/ >/dev/null 2>&1
    "$WORK/cmd_$i" "cd /root/private_data && rm -rf qqt-gpu-sim_r$i && mkdir -p qqt-gpu-sim_r$i && tar xzf jaxbomb_aggr.tgz -C qqt-gpu-sim_r$i" >/dev/null 2>&1
    echo "  ✓ [rank $i] 代码与 it102 底模部署完成"
  ) &
done
wait
...
echo "=== [4/4] 启动 9 节点 18 卡激进长训 (MASTER: ${MASTER}:29500, NW=$NW) ==="
# Rank 0
...
echo "=== 全部 $NW 个 Rank (18 张 DCU 卡) 已全量点火，新奖励激进长训正式启动！ ==="
