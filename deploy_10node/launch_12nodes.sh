#!/bin/bash
# 12 机 × 2 卡有损同步（Local SGD）训练编排（复制自 launch_10nodes.sh v2：
# 自检 + 健康检查 + 断点语义修正；节点数 = 节点文件行数，本脚本按 12 机准备）
#
# nodes_12x2.txt 兼容两种格式（可混用；# 开头为注释行；第 1 条 = rank 0，
...
#
# 用法:  bash launch_12nodes.sh nodes_12x2.txt [--deploy] [--resume]
...
#       rank0 先起 → 1-11 台 30s 内拉起 → 30s 后健康检查(空日志/Traceback 即中止)
#       → 提示用 watch_12nodes.sh 监控 + pull_ckpt_local.sh 拉快照。
set -u
NODES_FILE="${1:?用法: launch_12nodes.sh nodes_12x2.txt [--deploy] [--resume]}"
shift
...
[ "$NW" -eq 12 ] || echo "注意: 节点数 $NW ≠ 24（将按 WORLD_SIZE=$NW 继续；掉线降级时可少台重启）"
...
if [ "$ALL_OK" = "1" ]; then
  ...
  echo "=== 监控：bash watch_12nodes.sh nodes_12x2.txt（60s 刷新）==="
  echo "=== 拉 rank0 快照：bash pull_ckpt_local.sh nodes_12x2.txt ==="
else
