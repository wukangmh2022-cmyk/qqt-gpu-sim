...
// --------------------------------------------------------------------------
// 场景 6：十字交叉口角部半身位安全（横向水流与纵向水流交叉，角色处于对角半身位 -> 无伤！）
// --------------------------------------------------------------------------
{
  const sim = createCleanSim();
  // 横向水流穿过 row 5，纵向水流穿过 col 8
  sim.bombBlast[5 * W + 6] = 5; sim.fuse[5 * W + 6] = 1; sim.owner[5 * W + 6] = 1;
  sim.bombBlast[3 * W + 8] = 5; sim.fuse[3 * W + 8] = 1; sim.owner[3 * W + 8] = 1;

  // 玩家站在 (6.0, 7.9)（处于交叉口左下角）：
  // 上边缘 py0 = 6.0 - 0.42 = 5.58 > 5.5（未触及 row 5 横向水流中线 5.5）
  // 右边缘 px1 = 7.9 + 0.42 = 8.32 < 8.5（未触及 col 8 纵向水流中线 8.5）
  sim.pos[0] = 6.0; sim.pos[1] = 7.9;
  const hpBefore = sim.hp[0];
  sim.step([[MOVE_IDLE, 0], [MOVE_IDLE, 0]]);
  assert.strictEqual(sim.hp[0], hpBefore, '十字交叉口对角角部半身位应当完全无伤！');
