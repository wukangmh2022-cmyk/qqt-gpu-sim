...
MASTER_PORT=29560
echo "=== 启动实验 2 (Exp-2: 彻底回归初代纯净奖励 + Grad Mode 零漂移) ==="
echo "=== 参数: explore=0.0 brick=0.0 crate=0.5 (500M 退火) 纯胜负驱动 ==="
...
for i in $(seq 0 9); do
  /tmp/ndrun/cmd_$i "pkill -9 -f jax_bomb 2>/dev/null; pkill -9 -f train_real 2>/dev/null; pkill -9 -f test_distributed 2>/dev/null; fuser -k 29560/tcp 2>/dev/null; rm -rf /root/private_data/train_r*.log /root/private_data/qqt-gpu-sim/ckpt/* /root/private_data/qqt-gpu-sim/ckpt_local/* 2>/dev/null" >/dev/null 2>&1 || true
  sleep 0.3
...
echo "=== 代码已分发完毕，开始顺序拉起 Exp-2 纯净奖励训练进程 ==="
