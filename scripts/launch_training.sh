...
MASTER_PORT=29550
echo "=== 启动 10 节点 20 卡正式训练 (MASTER=$MASTER_IP:$MASTER_PORT) ==="
...
for i in $(seq 0 9); do
  /tmp/ndrun/cmd_$i "pkill -9 -f jax_bomb 2>/dev/null; pkill -9 -f train_real 2>/dev/null; pkill -9 -f test_distributed 2>/dev/null; fuser -k 29550/tcp 2>/dev/null; rm -rf /root/private_data/train_r*.log /root/private_data/qqt-gpu-sim/ckpt/* /root/private_data/qqt-gpu-sim/ckpt_local/* 2>/dev/null" >/dev/null 2>&1 || true
  sleep 0.3
