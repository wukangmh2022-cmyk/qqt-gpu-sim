...
assert.strictEqual(sim.crate[3 * W + 1], 0, 'Tick 3: 墙体未开放前绝不刷出道具');

// Tick 4 (400ms = 0.4s, 砖残骸消散，正式开放通行并刷出道具！)
sim.step([[MOVE_IDLE, 0], [MOVE_IDLE, 0]]);
...
assert.strictEqual(sim.crate[3 * W + 1], 1, 'Tick 4 (0.4s): 墙体开放的瞬间同步刷出道具！');

console.log('🎉 验证通过：砖块炸毁后严格在 0.4s（第 4 tick）开放通行，且道具在开放通行的同一刻刷出，彻底杜绝道具上墙！');
