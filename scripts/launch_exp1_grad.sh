...
MASTER_PORT=29540
echo "=== 启动实验 1 (Exp-1: 0e1c806 原版纯净复原 + Grad Mode 零漂移) ==="
echo "=== MASTER=$MASTER_IP:$MASTER_PORT WORLD_SIZE=10 ==="
...
for i in $(seq 0 9); do
  /tmp/ndrun/cmd_$i "pkill -9 -f jax_bomb 2>/dev/null; pkill -9 -f train_real 2>/dev/null; pkill -9 -f test_distributed 2>/dev/null; fuser -k 29500/tcp 29510/tcp 29520/tcp 29530/tcp 29540/tcp 2>/dev/null; rm -rf /root/private_data/train_r*.log /root/private_data/qqt-gpu-sim/ckpt/* /root/private_data/qqt-gpu-sim/ckpt_local/* 2>/dev/null" >/dev/null 2>&1 || true
  sleep 0.5
...
echo "=== 10 节点集群正式训练已拉起 (Grad Mode 零漂移) ==="
