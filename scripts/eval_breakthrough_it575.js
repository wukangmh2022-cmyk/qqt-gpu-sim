...
async function main() {
  console.log("正在加载 params_it00000575_ema 模型...");
  const m447 = await loadModel("params_it00000575_ema");
  ...
  for (let i = 0; i < 20; i++) {
    ...
    let t = 0;
    for (t = 0; t < 1000; t++) {
      const a0 = await m447.act(sim, 0, rng);
  ...
  for (let i = 0; i < 20; i++) {
    ...
    let t = 0;
    for (t = 0; t < 1000; t++) {
      const a0 = await m447.act(sim, 0, r0);
  ...
  for (let i = 0; i < 20; i++) {
    ...
    for (t = 0; t < 1000; t++) {
      const a0 = await m447.act(sim, 0, r0);
      const a1 = await m447.act(sim, 1, r1);
  ...
  console.log("RESULT_JSON:" + JSON.stringify({
    checkpoint: "params_it00000575_ema",
